        ...
        if (Request.QueryString["id"] != null)
        {
            ...
            if (!Page.IsPostBack)
            {
                using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["Entry9036"].ConnectionString))
                {
                    ...
                    ds.Clear();
                    SqlDataAdapter cm = new SqlDataAdapter("SELECT * FROM [Order] INNER JOIN [groupList] ON groupList.groupId = [Order].groupId WHERE (dbo.[Order].id = @id) AND empName = @UID", con);
